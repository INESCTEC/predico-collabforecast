...
def validate_query_params(
    ...
    if challenge_id is not None:
        try:
            ...
            raise exceptions.ValidationError(
                "Query param 'challenge_id' must be a valid uuid."
            ) from ex
    if submission_id is not None:
        try:
            ...
            raise exceptions.ValidationError(
                "Query param 'submission_id' must be a valid uuid."
            ) from ex
    ...
    if challenge_use_case is not None:
        ...
        if challenge_use_case.lower() not in lbl:
            raise exceptions.ValidationError(
                f"Query param 'use_case' must be "
                f"one of the following {lbl}"
            )
    ...
    if ((open_only is not None)
        ...
        raise exceptions.ValidationError(
            "Query param 'open_only' must be a boolean (true/false)"
        )

    if ((pending_only is not None)
            and (pending_only.lower() not in ['true', 'false'])):
        raise exceptions.ValidationError(
            "Query param 'pending_only' must be a boolean (true/false)"
        )
