...
def random_coef_VAR(n_lags, n_agents, ar_scale=5, sparse_proportion=0.5):
    ...
    for i1 in range(n_lags):  # for each lag, generate coefficients
        ...
        # introduce sparsity
        grid = np.random.binomial(1, sparse_proportion, size=(n_agents, n_agents))
        A[grid == 1] = 0
    ...
    for s in range(-1, n_lags - 1):
        ...
        if s >= 0:
            for k in range(s + 1):
                all_phi[:, :, s + 1, k] = all_phi[:, :, s, k] - np.matmul(
                    all_phi[:, :, s + 1, s + 1], all_phi_star[:, :, s, s - k]
                )  # noqa
                all_phi_star[:, :, s + 1, k] = all_phi_star[:, :, s, k] - np.matmul(
                    all_phi_star[:, :, s + 1, s + 1], all_phi[:, :, s, s - k]
                )  # noqa

        # These are not needed in the last round because only coefficient
        # matrices will be returned.
        if s < n_lags - 2:
            Sigma_next = Sigma - np.matmul(
                all_phi[:, :, s + 1, s + 1],
            ...
            L = cholesky(Sigma).T
    coefs_var = (
        all_phi[:, :, -1].flatten(order="F").reshape((n_agents, n_lags * n_agents)).T
    )
    return coefs_var
