...
class PostgresDB:
    ...
    def get_db_instance(config_name="default"):
        if PostgresDB.instances.get(config_name, None) is None:
            PostgresDB.instances[config_name] = PostgresDB(config_name=config_name)
...
if __name__ == "__main__":
    ...
    data = db.read_query_pandas(
        "select * from raw_data where resource_id='bob-asd-3-23';"
    )
    forecasts = data[
        ["datetime", "value", "unit", "registered_at", "user_id", "resource_id"]
    ]
